...
{name:'Medium Voltage Switchgear A and B',desc:'All items checked via EPMS. Check that all breakers are in normal alignment, FLoReS is in AUTO, relay status is good, and free of alarms. Check Controls UPS Voltage (110 Vdc - 140 Vdc). Verify no audible or visual alarms present.',cols:['Breaker Status','FLoReS','UPS','Audible/Visual'],type:'ok_issue',cheatSheet:'NOTE: All items to be checked via EPMS | System voltage: 34.5 kV (live ~34,746 V) | All breakers: Normal alignment | FLoReS: AUTO | Controls UPS: 110-140 Vdc | Relay status: Good, no trip flags | No audible or visual alarms'
  ...
  {name:'IW Room',desc:'Check all equipment is in auto and free of alarms. Inspect for leaks and debris. Verify pump skids, MIOX system, and IW tank levels. See Reference Values for specific ranges.',cols:['Pump Skid 1','Pump Skid 2','Miox Skid','Tanks','BMS Panel'],type:'exp_unexp',cheatSheet:'Pump pressure setpoint: 47 PSI (EPMS) | Tank low alarm: <70% operating level (60s delay) | Tank overflow: 4in below overflow level | Tank discrepancy alarm: >10% between sensors | Dump/Flush pause: tanks <85% | Loss of flow alarm: <30 GPM for 120s AND <20 PSI | Alarm reset: pressure >30 PSI | Chlorine: OX skid maintains setpoint | UV: Continuous, rotates tanks every 2hr | Pump rotation: Lead/standby every 168hr (7 days) | Dump cycle: Every 7 days | Modes: Tank Water (primary), Utility Water (secondary), Maintenance, Shutdown'},
  {name:'Fire Riser Room',desc:'Verify all valves are in correct position, no leakages, no unexpected alarms on FCP. Check nitrogen generator, air compressor, and all zone pressures. See Reference Values for specific ranges.',cols:['Valves','Pressure','FACP','Nitrogen Gen'],type:'exp_unexp',cheatSheet:'Sprinkler heads: 175-200°F activation | Line pressure: 140-150 PSI (site-specific) | Nitrogen generator: 20-62 PSI | Air compressor: 120-175 PSI | Zone water pressure: 125-150 PSI | Zone air pressure: 15-30 PSI | All valves: Correct position per tag | FCP: No alarms | No leaks'},
...
var zoneMap = [
  ...
  {id:'er-1',label:'Electrical Room 1.1-1.6',sIdx:4,type:'electrical'},
  {id:'dahu-1-odds',label:'DAHU Gallery 1 (Odds)',sIdx:5,type:'dahu'},
  {id:'dahu-1-evens',label:'DAHU Gallery 1 (Evens)',sIdx:6,type:'dahu'},
  {id:'roof',label:'Roof',sIdx:7,type:'roof'},
  {id:'iw-room',label:'IW Room',sIdx:8,type:'fire'},
  {id:'fire-riser',label:'Fire Riser Room',sIdx:9,type:'fire'},
  {id:'dahu-2-evens',label:'DAHU Gallery 2 (Evens)',sIdx:10,type:'dahu'},
  {id:'dahu-2-odds',label:'DAHU Gallery 2 (Odds)',sIdx:11,type:'dahu'},
  {id:'er-2',label:'Electrical Room 2.1-2.6',sIdx:12,type:'electrical'},
  {id:'gen-dh2',label:'Gen Yard DH2',sIdx:13,type:'generator'},
...
var sectionItemColors={0:'item-zone-office',1:'item-zone-mv',2:'item-zone-gen',3:'item-zone-diesel',4:'item-zone-er',5:'item-zone-dahu',6:'item-zone-dahu',7:'item-zone-roof',8:'item-zone-fire',9:'item-zone-fire',10:'item-zone-dahu',11:'item-zone-dahu',12:'item-zone-er',13:'item-zone-gen',14:'item-zone-datahall'};
...
function renderZoneList(){
  ...
  for(var i=0;i<zoneMap.length;i++){
    ...
    if(zoneFindings.length>0){
      ...
      var showMax=Math.min(zoneFindings.length,3);
      for(var fi=0;fi<showMax;fi++){html+='<div class="zone-finding-item"><strong>'+escHtml(zoneFindings[fi].item)+'</strong><br>'+escHtml(zoneFindings[fi].note)+'</div>';}
      if(zoneFindings.length>3)html+='<div style="font-size:11px;color:var(--muted);margin-top:4px">+'+(zoneFindings.length-3)+' more \u2014 see Findings tab</div>';
      html+='</div>';
    }
    html+='</div></div></div>';
  }
...
html+='</div></div></div>';
  ...
  if(sec.type==='notes_only'){
    ...
    if(!('ontouchstart' in window)){html+='<div style="background:rgba(6,182,212,0.1);border:1px solid var(--accent);border-radius:var(--radius-sm);padding:10px 14px;margin-bottom:10px;font-size:12px;color:var(--accent);text-align:center"><strong>Desktop Mode:</strong> Use the OK / Issue buttons on each item</div>';}
html+='<div class="item-list">';
    var sCols=sec.cols||[];for(var idx=0;idx<sCols.length;idx++){
      ...
      html+='</div>';
      if(!('ontouchstart' in window)&&!viewOnlyMode){var dOk=sec.type==='exp_unexp'?'expected':'ok';var dIssue=sec.type==='exp_unexp'?'unexpected':'issue';html+='<div class="desktop-item-btns" style="display:flex;gap:6px;margin-top:6px"><button onclick="event.stopPropagation();markItem('+currentSection+','+idx+',\x27'+dOk+'\x27)" style="flex:1;padding:6px 10px;border-radius:var(--radius-sm);border:1px solid var(--green);background:'+(item.status===dOk?'var(--green)':'transparent')+';color:'+(item.status===dOk?'#fff':'var(--green)')+';font-size:12px;font-weight:600;cursor:pointer;min-height:32px">\u2713 '+(sec.type==='exp_unexp'?'Expected':'OK')+'</button><button onclick="event.stopPropagation();markItem('+currentSection+','+idx+',\x27'+dIssue+'\x27)" style="flex:1;padding:6px 10px;border-radius:var(--radius-sm);border:1px solid var(--red);background:'+(item.status===dIssue?'var(--red)':'transparent')+';color:'+(item.status===dIssue?'#fff':'var(--red)')+';font-size:12px;font-weight:600;cursor:pointer;min-height:32px">\u26A0 '+(sec.type==='exp_unexp'?'Unexpected':'Issue')+'</button></div>';}
      html+='</div>';
      if(item.status==='issue'||item.status==='unexpected'){
...
function loadRecentRounds(filterBuilding){
  ...
  var buildings=filterBuilding==='ALL'?buildingList:[filterBuilding];
